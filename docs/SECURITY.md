...
- **Authentication of the unencrypted codec prefix (when codec-aware partial encryption is enabled).** When a `Codec` is configured on a `StreamsMsg`, the first N bytes of each encoded frame are left in the clear so the SFU can route by frame type and browser decoders can fail gracefully on key mismatch. The authentication status of these prefix bytes depends on the deployment phase:

  **Phase 1 (LIVE — current):** The prefix is **not** included in AES-GCM AAD. The sender emits `AAD = header` (the original format). An attacker who can modify wire bytes (e.g. the SFU itself, or an on-path observer) can corrupt the codec header without detection by the AEAD layer. The decoded prefix will be whatever the attacker substituted — the rest of the frame (everything after the prefix) remains authenticity-protected as usual. This trade-off is intentional and mirrors the approach taken by LiveKit's `FrameCryptor`. If you need every byte to be authenticated, do not set a codec (the default `N=0` full-encryption path remains unchanged and provides complete AEAD coverage).

  **Phase 1 receiver tolerance (LIVE — current):** The receiver accepts three AAD forms: `header` (original/phase-1 sender), `prefix || header` (intermediate `b6ded9d` form), and `be16(prefix.length) || prefix || header` (canonical/phase-2 target). The first successful decrypt caches the format per `(epoch, peerIndex)` so subsequent frames use exactly 1 AEAD attempt. For VP9/AV1, the receiver also tolerates both `N=0` (old/phase-1 sender) and `N=1` (new/phase-2 sender) prefix lengths. This means a phase-1 receiver can interop with both phase-1 and phase-2 senders without any configuration change.

  **Phase 2 (FUTURE — not yet live):** The sender will switch to the canonical AAD form (`be16(prefix.length) || prefix || header`) and VP9/AV1 `N=1`. This makes the unencrypted prefix tamper-evident — an attacker who modifies the prefix causes an AEAD auth failure. The canonical form uses a big-endian 16-bit length prefix to avoid the ambiguity of the intermediate `prefix || header` form (where two different `(prefix, header)` pairs can produce identical AAD bytes). Phase 2 will be a **breaking change** (major version bump) because the sender's wire format changes. Phase 1 receivers already accept the canonical form, so the phase 2 switch does not break interop with deployed phase 1 receivers.
